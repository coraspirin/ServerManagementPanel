...
export async function seedMockHistory(): Promise<string | null> {
  ...
  const db = getDb();
  const existing = db.prepare("SELECT COUNT(*) AS n FROM metrics_1d").get() as { n: number };
  ...
  const insertRaw = db.prepare(
    `INSERT OR REPLACE INTO metrics_raw (host_id, metric, label, ts, value) VALUES (1, ?, ?, ?, ?)`,
  );
  ...
  try {
    ...
    for (let ts = now - 70 * 60; ts <= now; ts += collect) {
      ...
      for (const sample of sampleAt(fx, aligned)) {
        insertRaw.run(sample.metric, sample.label ?? "", aligned, sample.value);
        rows++;
      }
    }
    ...
    for (const tier of TIERS) {
      ...
      for (let bucket = start; bucket < end; bucket += tier.bucket) {
        ...
        for (let i = 0; i < tier.subSamples; i++) {
          for (const sample of sampleAt(fx, bucket + i * step)) {
