 ...
 *
 * Metin SÖZLÜKTE, veritabanında değil: bir ekranın ne yaptığı koddan türeyen
 * bir gerçek, ikisi birlikte değişmeli ve "bu açıklama ne zaman değişti"nin
 * cevabı git geçmişi olmalı.
 *
 * Üç başlık bilinçli: kullanıcının sorduğu şey hep aynı üç soru — bu ekran ne
 * işe yarar, arkasında ne dönüyor, neye dikkat etmeliyim.
 *
 * Ayarların alt sayfaları BURADA YOK: metinleri `settings.groups`tan geliyor
 * (bkz. PageHelp). İki ayrı açıklama tutmak, ikisinin zamanla ayrışması demek.
 */
