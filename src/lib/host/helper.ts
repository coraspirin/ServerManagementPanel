...
export function helperConfigured(): boolean {
  ...
  if (!isLocalHost(hostId)) {
    // Ajan hello'da kendi HELPER_SECRET'ı olup olmadığını bildiriyor.
    const host = getHost(hostId);
    return host?.agentType === "agent" && host.capabilities?.helper === true;
  }
  return (process.env.HELPER_SECRET ?? "") !== "";
}

/**
 * Uzak sunucuda helper'a yalnızca bu alanlar iletilir; güvenlik duvarı,
 * fail2ban ve cron yalnızca yerel sunucuda yönetiliyor (menüde de gizli).
 * Ajan aynı listeyi ayrıca uyguluyor — merkez atlatılsa bile.
 */
export const REMOTE_HELPER_PREFIXES = ["power.", "service.", "compose.", "journal.", "shell."] as const;

export function remoteHelperAllowed(action: string): boolean {
  return REMOTE_HELPER_PREFIXES.some((prefix) => action.startsWith(prefix));
}

/**
 * Çoklu sunucu: etkin sunucu uzaksa istek o sunucudaki helper'a ajan
 * üzerinden iletilir. Uzak sunucunun HELPER_SECRET'ı ve izin listesi o
 * sunucuda kalır; panel yalnızca ajana "şu eylemi iste" der. Ajan istemcisi
 * dinamik yükleniyor: helper pek çok yerden içe aktarılıyor ve istemcinin
 * bağımlılıklarını her birine taşımanın gereği yok.
 */
async function callRemoteHelper(
  hostId: number,
  action: HelperAction,
  ...
  const host = getHost(hostId);
  if (!host || host.agentType !== "agent" || !remoteHelperAllowed(action)) {
    return { ok: false, error: serverT("hosts.errors.unsupported") };
  }
  const { agentCall } = await import("@/lib/agent/client");
  try {
    return await agentCall<HelperResponse>(host, "helper.call", [action, args, actor, timeoutMs, requestId], {
      // Ajanın helper'ı beklemesine ek pay: yanıt yolda kaybolmasın.
...
export async function callHelper(
  ...
  const hostId = currentHostId();
  if (!isLocalHost(hostId)) return callRemoteHelper(hostId, action, args, actor, timeoutMs, requestId);
